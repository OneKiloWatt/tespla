 ...
 * テスト結果記入 (/result/new)
 *
 * 計画の終了状態（home_ended）から遷移して、各教科の点数とメモを記録。
 */
export default function ResultEntryPage() {
  ...
  const onSubmit = (values: TestResultInput) => {
    // TODO: upsertPlan({ ...plan, result: { ...values, recordedAt: ... } })
    console.log('save result', values);
    router.push('/home');
  };
    ...
    <>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex-1 flex flex-col">
        <main className="flex-1 overflow-y-auto p-[18px] pb-4">
          <div className="text-[11px] text-text-mid">{plan.testName}</div>
        ...
        <div className="px-4 py-3 border-t border-divider bg-bg-base flex gap-2 shrink-0">
          <Button type="button" variant="secondary" className="flex-1" onClick={() => router.push('/home')}>
            あとで
          </Button>
